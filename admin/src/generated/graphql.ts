...
export type RestaurantsQuery = (
  ...
  & { restaurants: Array<(
    { __typename?: 'Restaurant' }
    & Pick<Restaurant, 'id' | 'createdAt' | 'name' | 'email' | 'businessPhone' | 'reservationPhone' | 'url' | 'address' | 'city' | 'postCode' | 'country' | 'currency' | 'priceRange' | 'diningStyle' | 'vat' | 'serviceCharge' | 'hasParking' | 'hasPartyFacilities' | 'hasKidsZone' | 'isTakeaway' | 'isDelivery' | 'isReservation' | 'isOrdering' | 'isAutoAcceptOrder' | 'isAutoAcceptReservation' | 'updatedAt'>
  )> }
);
...
export const useRestaurantsQuery = <
    ...
    useQuery<RestaurantsQuery, TError, TData>(
      ...
      fetcher<RestaurantsQuery, RestaurantsQueryVariables>(dataSource.endpoint, dataSource.fetchParams || {}, RestaurantsDocument, variables),
      options
    );